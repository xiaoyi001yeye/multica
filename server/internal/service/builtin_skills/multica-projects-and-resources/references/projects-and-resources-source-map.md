...
- `project resource update` merges shortcut edits with existing `resource_ref` so a partial edit does not clobber required fields; non-JSON `--ref` updates `github_repo.resource_ref.ref`. Repository URLs and local-directory daemon IDs are immutable; changing either is remove + add.
...
- `server/internal/daemon/execenv/runtime_config.go` renders repository checkout ref, provider, role, default-branch hint, PR/MR guide, and local-directory binding context into the agent brief.
