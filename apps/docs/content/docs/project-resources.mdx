...
description: Attach Git repositories and local directories to a project so agents can pick them up as scoped context.
---

import { Callout } from "fumadocs-ui/components/callout";

A **Project Resource** is a typed pointer — currently a Git repository or a path on one of your machines — attached to a [project](/workspaces). When an [agent](/agents) runs against an issue inside that project, the daemon automatically writes the project's ordered resource list into the agent's working directory and into its [meta-skill](/skills) prompt.

The result: the agent knows which repository to check out or which local directory to work in, without anyone copy-pasting context into the issue body.
...
- The supported resource types are `github_repo` and `local_directory`.
...
```json
{
  ...
  "resource_ref": {
    ...
    "provider": "github",
    "role": "backend",
    "pr_creation_guide": "Create a branch from main and include the issue key in the PR title."
  }
}
```

Despite the internal type name, repository URLs may point to GitHub, GitLab, self-hosted Git, or another Git server. Provider, role, description, and PR/MR guide are optional execution hints surfaced to the agent.
...
On **Desktop**, attach a directory from the native folder picker:
...
On the **Web app**, select a local runtime or machine and enter its absolute path manually. Cloud runtimes are excluded. An offline local runtime may still be selected; the path is shown as unverified and is checked when a task starts. A project may have only one local directory per daemon.
...
In the **Web** or **Desktop** app, opening *New project* shows a **Repos** pill alongside Status / Priority / Lead. Selecting workspace-bound repositories (or pasting an ad-hoc Git URL) attaches them as `github_repo` resources when the project is created. The same picker can attach one local directory; remote repositories remain the fallback on other machines.
...
# Update editable metadata or remove and re-add an immutable repository URL.
multica project resource update <project-id> <resource-id> --default-branch-hint develop
```
...
- **Git repository**: https://github.com/multica-ai/multica
  - Provider: GitHub
  - Role: backend
  - Checkout ref: `release/v2`
  - Default branch hint: `main`
  - Checkout: `multica repo checkout https://github.com/multica-ai/multica --ref release/v2`
...
- **Caching / sync.** `github_repo` is metadata — checkout still happens via `multica repo checkout` on demand.
