# Project 资源管理增强功能需求文档

## Problem Statement

当前 Multica 的 Project 已经可以挂 `github_repo` / `local_directory` 类型的资源，但产品表达和页面能力还不够完整。用户真正想完成的是：

在一个 Project 下管理与代码执行相关的资源，让 Agent 能根据 issue 自动知道应该在哪些 Git 仓库或本地目录工作，并在具备凭证时提交 commit、push、创建 PR/MR。

V1 的资源管理增强只覆盖两种现有资源类型：`github_repo` 和 `local_directory`。不新增 Notion、文档、外部 URL、上传文件等其他资源类型。
...
4. 作为 Agent 使用者，我想调整资源顺序，以便 Agent 优先看到更重要的仓库或目录。
...
- 功能命名：使用 **Project Resources / 项目资源**。页面文案限定为 Git 仓库和本地目录，不承诺其他资源类型。
- V1 类型范围：只支持现有 `github_repo` 和 `local_directory`。
- 内部类型名：保留 `resource_type = github_repo`，用户可见文案统一称为 Git repository / Git 仓库。
- 资源优先级：不新增 `primary` 语义。Multica 现有规则按 `position ASC, created_at ASC` 排序，V1 继续用排序表达推荐阅读/checkout 顺序。
- 排序交互：V1 支持轻量上移/下移，不做拖拽。
- 排序范围：底层统一排序；页面可分组展示 Git repositories 和 Local directories，并在同组内调整顺序。`local_directory` 是否成为实际工作树由 daemon 匹配决定，不由排序决定。
- `github_repo.url`：V1 不作为普通可编辑字段。输错或换仓库时删除旧资源再新增，避免同一个资源 ID 指向完全不同的仓库。
- `local_directory.local_path`：允许在同一 daemon 下“重新选择目录”或手动更新路径；`daemon_id` 不作为普通可编辑字段。换机器/运行时等价于删除旧资源，在目标机器上新增。
- Web 添加本地目录：允许 Web 端手动添加 `local_directory`，但必须选择目标本地运行时/机器并输入绝对路径。
- 离线运行时：允许选择离线本地运行时保存路径，资源状态显示为未验证；任务启动时 daemon 再做最终校验。
- 云端运行时：V1 不允许为云端运行时绑定手动输入的本地路径，除非未来云端运行时有明确文件系统映射能力。
- 验证状态：V1 不新增数据库字段持久化 verification state。即时校验结果只作为 UI 派生状态；daemon 启动任务前仍是最终校验者。
- 路径可见性：V1 不新增字段级权限。能查看 Project 资源的成员可在详情中看到完整路径；列表和任务状态优先展示 label / 脱敏路径。
- `local_directory` Git 要求：不强制目录必须是 Git 仓库，只要求是安全、可读写的目录。
- `local_directory` 结构：保持原生定义 `{ local_path, daemon_id, label? }`，不增加 `repo_url`。
- 混用语义：`local_directory` 是 Project + daemon 级本地工作目录覆盖，不映射到某个具体 Git repo。匹配当前 daemon 时本地目录优先；不匹配时回退到 `github_repo` worktree 流程。
- 唯一性：同一 Project + 同一 daemon 最多一个 `local_directory`。不同 daemon 可以各自绑定一个目录。

## Proposed Features

### 1. Project 页面增加“项目资源”区块

#### 需求

在 Project Detail 页面增加更明确的“Project Resources / 项目资源”区块。V1 只展示和管理两类资源：Git 仓库与本地目录。
...
- `resource_ref` 可在 V1 增加少量 Git 仓库展示字段，但不新增 `primary`：
...
一个 Project 可添加多个 Git 仓库和本地目录，并支持资源排序。排序表达 Agent 上下文中的推荐阅读/checkout 顺序。

#### 实现思路

- 现有 `project_resource.position` 可用于排序。
- V1 新增 UI 上移/下移按钮，不做拖拽。
- 底层仍使用统一的 `project_resource.position`，列表按 `position ASC, created_at ASC`。
- 页面可以分组展示 Git repositories 和 Local directories，但不拆分底层排序字段。
- 同组内上移/下移即可满足 V1；不要求跨组拖动。
- Agent prompt 和 `.multica/project/resources.json` 保留统一排序语义。
- V1 不引入 `primary` repo 字段和唯一性约束。若未来需要显式主仓库，再单独设计。
...
- UI 增加编辑入口。
- `github_repo.url` 不作为普通可编辑字段；换 URL 通过删除旧资源再新增完成。
- Agent prompt 中明确：
...
### 7. 本地目录作为 Project 级工作目录覆盖

#### 需求

用户既可以关联远程 Git 仓库，也可以指定某个本地运行时/机器上的目录作为该 Project 的执行目录。本地目录适用于：
...
- V1 保持原生 `local_directory` 结构，不增加 `repo_url`，也不绑定到具体 Git repo。
- `local_directory` 是 Project + daemon 级本地工作目录覆盖。
...
Project 页面需要把 `local_directory` 作为可直接添加的一等资源，而不是只作为 Git repo 的隐藏补充。用户可以在 Project Detail 页面添加本地目录资源，保存后该 Project 下的 Agent 任务会在匹配 daemon 上原地执行。
...
- Web 端允许手动添加本地目录：用户先选择本地运行时/机器，再输入绝对路径和 label。
- Web 端可以选择在线或离线的本地运行时；离线保存后状态显示为未验证。
- Web 端不允许为云端运行时绑定手动输入的本地路径。
- 添加前如果目标 daemon 在线，调用 daemon 本地校验，确认路径存在、是目录、可读、可写；如果 daemon 离线，仅保存并标记未验证。
...
- Web 端手动输入路径时，若所选本地运行时在线，可请求目标 daemon 做即时校验；若离线则保存为未验证。
...
- V1 不新增 verification state 数据库字段。即时检测结果作为 UI 派生状态，不能替代 daemon 启动任务前的最终校验。
- V1 不强制 `local_directory` 必须是 Git 仓库；如果任务需要 commit 或 PR/MR，UI 可以提示用户选择 Git checkout 内的目录。
...
- 明确提示：同一 Project + daemon 只有一个本地工作目录覆盖。多 repo Project 需要选择能代表该 Project 的目录，例如 monorepo 根目录；多 repo 到多目录的精细映射不属于 V1。
...
用户需要能维护资源的展示信息和执行提示，但避免把关键身份字段改成含义完全不同的资源。
...
- `github_repo` 可编辑：
  ...
  - role / 用途
  - position / 排序
- `github_repo.url` 不作为普通可编辑字段。输错或换仓库时删除旧资源，再新增新资源。
- `local_directory` 可编辑：
  - label
  - `local_path`，语义为“重新选择目录”或“更新路径”
  - position / 排序
- `local_directory.daemon_id` 不作为普通可编辑字段。换机器/运行时等价于删除旧资源，在目标本地运行时上新增。

### 14. PR/MR 创建能力：短期 Prompt 驱动
...
Project 下的 issue 默认继承 Project 资源；单个 issue 后续也可以覆盖或指定具体资源。

#### 实现思路

- V1：只做 Project 级别，不做 issue 覆盖。
- V2：issue metadata 或 issue-resource link 支持指定 repo。
- UI 在 issue 创建时可选择“Target resource”或“Target repo”。
- Agent prompt 优先级：
  1. issue 指定 repo
  2. project resource list in order
...
- 后端校验允许新增少量可选展示字段，但继续兼容旧 `{ url, default_branch_hint }`。
- V1 不引入 `primary` repo。资源优先级沿用 `position ASC, created_at ASC`。
- V1 继续使用现有 `local_directory` resource type，并把它作为 Project 页面上的一等资源入口。
- V1 不给 `local_directory` 增加 `repo_url`，保持原生 `{ local_path, daemon_id, label? }`。
- Web 端可手动添加 `local_directory`：选择本地运行时/机器并输入绝对路径；桌面端优先使用文件夹选择器。
- Web 端允许保存离线本地运行时路径并标记未验证；不允许绑定云端运行时。
- V1 不持久化 verification state，daemon 启动任务前做最终校验。
- V1 不要求 `local_directory` 必须是 Git 仓库。
- 同一 Project + 同一 daemon 仍最多允许一个 `local_directory`，该限制由 API 和 UI 双重表达。
- Agent prompt 增强 Project Context，把资源顺序、role、default branch 写清楚。
- Agent prompt 增强 Local directory Context，把本地目录 label、绑定运行时/机器、远程 fallback 关系写清楚。
...
- API 测试：同一 Project + daemon 重复创建 `local_directory` 返回冲突。
- 前端测试：Project Resources 区块可添加、删除、编辑默认分支、展示 provider。
- 前端测试：资源上移/下移更新 `position`，Agent context 中顺序稳定。
- 前端测试：桌面端可通过文件夹选择器添加 `local_directory`。
- 前端测试：Web 端可选择本地运行时/机器并输入路径添加 `local_directory`；离线本地运行时保存后显示未验证。
- 前端测试：Web 端不可为云端运行时绑定本地路径。
- 前端测试：同一 daemon 已有本地目录时，新增按钮置灰或保存被拦截并显示原因。
...
- Prompt 测试：资源顺序、default branch、role 会进入 Agent context。
...
- V1 不做代码权限审计或文件级锁。
- V1 不新增非代码资源类型，例如 Notion、文档、外部 URL、上传文件。
- V1 不新增 `git_repository` resource type，也不迁移现有 `github_repo`。
- V1 不新增 `local_directory.repo_url` 或 repo-to-directory 精细映射。
- V1 不新增 `primary` repo 语义。
- V1 不持久化 local directory verification state。
...
1. **V1：Project Resources UI 增强**
   - 明确项目资源概念，只支持 `github_repo` 和 `local_directory`。
   - 支持 GitLab 展示、默认分支、role、资源排序。
2. **V2：Local directory 一等入口**
   - Project 页面支持添加、展示、重命名、重新选择路径、删除 `local_directory`。
   - 桌面端接文件夹选择器和本地路径校验；Web 端支持选择本地运行时/机器并手动输入绝对路径。
